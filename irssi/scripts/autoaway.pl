...
# (c) 2013 Jan Seidl (jseidl@wroot.org)
...
# Nick suffix appending added by Jan Seidl <jseidl@wroot.org> on 2012-07-24
...
sub auto_timeout {
  ...
  foreach my $server (Irssi::servers()) {
      ...
      # Nick Mod: Checks if nick is not already modded (has suffix)
      #		  adds suffix if so
      $cur_nick = $server->{nick};
      if (substr($cur_nick, length($nick_suffix)) != $nick_suffix) { 
	      $away_nick = "$cur_nick$nick_suffix";
	      $server->command("/NICK $away_nick");
      }# end if
  }
...
sub reset_timer {
   if ($autoaway_state eq 1) {
     ...
     foreach my $server (Irssi::servers()) {
         $server->command("/AWAY");
	 # Nick Mod: Checks if nick was modded (has suffix)
	 # 	     and resets back to original nick
	 $cur_nick = $server->{nick};
	 if (substr($cur_nick, length($nick_suffix)) == $nick_suffix) { 
		 $orig_nick = substr($cur_nick, 0, (length($nick_suffix)*-1));
		 $server->command("/NICK $orig_nick");
	 }#end if
     }
